...
def get_box_points(points, pose=None):
    ...
    d_w_p = -np.sort(-np.arange(0, l / 2, l / (point_num * 2))[0:point_num])
    d_w_n = np.arange(-l / 2, 0, l / (point_num * 2))[0:point_num]

    d_l_p = np.arange(w / 2, w * (4 / 7), (w * (4 / 7) - w / 2) / (point_num * 2))[0:point_num]
    d1 = np.zeros(shape=(point_num, 4))
    d1[:, 0] = w / 2
    d1[:, 1] = l_vector
    d1[:, 2] = z_diag
    ...
    d4[:, 2] = d_z_n
    d4[:, 3] = i
    point_mat = np.mat(np.concatenate((z1, z2, z3, z4, w1, w2, w3, w4, l1, l2, l3, l4, d1,d2)))
    # point_mat = np.mat(np.concatenate(( l1, l2)))
...
def get_box_points_nuscenes(points, pose=None):
    """
    box to points
    :param points: (7,),box
    :param pose:
    ...
    d4[:, 3] = i
    point_mat = np.mat(np.concatenate((z1, z2, z3, z4, w1, w2, w3, w4, l1, l2, l3, l4, d1,d2)))
    # point_mat = np.mat(np.concatenate(( l1, l2,d1,d2)))
