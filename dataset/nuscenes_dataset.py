...
from .kitti_data_base import read_tracking_label
...
class nuscenesTrackingDataset:
    def __init__(self,config,nusc,load_image=False,load_points=False,type=["car"],label_path=None):
    ...
    def __getitem__(self,input_sample):
        ...
        item, my_scene, current_token, current_sample = input_sample
        ...
        if self.config.load_data == "True":
            ...
            points = LidarPointCloud.from_file(pcl_path)
        else:
            points = None
        if self.config.load_data == "True":
            image_path = os.path.join(self.config.dataset_path, camera_data['filename'])
